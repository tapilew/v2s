...
const OUT = `${HERE}/sheet-results-v2.jsonl`;
...
const NEW_SCHEMA = {
	type: "object",
	properties: {
		elementos: { type: "array", minItems: 1, maxItems: 20, items: text(80) },
		titulo: text(40),
	...
	required: ["elementos", "titulo", "columnas", "filas"],
};
const updateSchema = (n) => ({
	type: "object",
	properties: { elementos: { type: "array", minItems: 1, maxItems: 20, items: text(80) }, filas: { type: "array", maxItems: 20, items: { type: "array", minItems: n, maxItems: n, items: text(60) } } },
	required: ["elementos", "filas"],
});
const COMMON =
	"Conviertes lo que alguien dice en una hoja de cálculo. Primero lista en elementos cada cosa o evento mencionado, copiando las palabras del texto, sin omitir ninguno. Luego una fila por cada elemento, en el mismo orden. Concepto es la cosa comprada, pagada o recibida, nunca el día. Usa solo lo que se dijo; deja \"\" cuando un dato no se mencionó; nunca inventes números. " +
...
const HARNESS = {
	finanzas: {
		...
		system: `${COMMON} Es sobre dinero: elige columnas como Fecha, Concepto, Tipo (Ingreso, Gasto o Ahorro), Categoría, Monto, Método. Monto como número sin símbolo.\nEjemplo. Texto: "Compré el súper por 80 con tarjeta y le pasé 20 a mi cuenta de ahorros." Hoja: {"elementos":["Compré el súper por 80 con tarjeta","le pasé 20 a mi cuenta de ahorros"],"titulo":"Movimientos","columnas":["Fecha","Concepto","Tipo","Categoría","Monto","Método"],"filas":[["2026-09-10","Supermercado","Gasto","Alimentación","80","Tarjeta"],["2026-09-10","Cuenta de ahorros","Ahorro","Ahorro","20",""]]}`,
		texts: [
	...
	salud: {
		...
		system: `${COMMON} Es sobre salud: equipos médicos, consultas, pacientes, medicamentos con dosis, vía y frecuencia, signos vitales. Nunca agregues diagnósticos, dosis ni valores que no se dijeron.\nEjemplo. Texto: "En la Clínica Norte vi un ecógrafo Siemens de unos cinco años." Hoja: {"elementos":["un ecógrafo Siemens de unos cinco años"],"titulo":"Equipos Clínica Norte","columnas":["Cliente","Modalidad","Cantidad","Marca","Antigüedad","Estado"],"filas":[["Clínica Norte","Ultrasonido","1","Siemens","5","Estimado"]]}`,
		texts: [
